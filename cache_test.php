		...
		if ($temp = cach_get("^CacheV3rs10n",$uid_session)) {
			...
			if (isset($_POST['sbCset'])) {
				if ($_POST['stCount']>1000000) {
					$_SESSION['res_cach_cset'] = 'Максимальное количество<br>повторов - 1млн.';
				} else {
			...
			if (cach_exec('set ^CacheN4m3sp4s3("'.$uid_session.'") = $znspace')) {
				if ($temp = cach_get("^CacheN4m3sp4s3",$uid_session)) {
					...
					echo '<input name="sbmNmsp" type="submit" value="Сменить"><br></form>';
					echo '<form method="POST"><table border=1 width=900><caption>Функции добавляемые в PHP</caption>';
					if(isset($_SESSION['error'])) {
						echo '<p style="color:#FF0000">'.$_SESSION['error'].'<p>';
						unset($_SESSION['error']);
					}
					echo '<tr><th>Функция PHP</th><th>Параметры</th><th>Результат</th></tr>';
					if (isset($_POST['sbset'])) echo '<tr style="background-color:#6b8e23">'; else echo '<tr>';
					echo '<td>cach_set</td><td>Имя: <input name="stGlNm" value="'.$_SESSION['stGlNm'].'"><br>Индекс1: <input name="stInx1" value="'.$_SESSION['stInx1'].'"><br>Индекс2: <input name="stInx2" value="'.$_SESSION['stInx2'].'"><br>Значение: <input name="stVal" value="'.$_SESSION['stVal'].'"><br><input name="sbset" type="submit" value="Выполнить"></td><td>';
					var_dump($_SESSION['res_cach_set']);
					echo'</td></tr>';
					if (isset($_POST['sbget'])) echo '<tr style="background-color:#6b8e23">'; else echo '<tr>';
					echo '<td>cach_get</td><td>Имя: <input name="gtGlNm" value="'.$_SESSION['gtGlNm'].'"><br>Индекс1: <input name="gtInx1" value="'.$_SESSION['gtInx1'].'"><br>Индекс2: <input name="gtInx2" value="'.$_SESSION['gtInx2'].'"><br><input name="sbget" type="submit" value="Выполнить"></td><td>';
					var_dump($_SESSION['res_cach_get']);
					echo '</td></tr>';
					if (isset($_POST['sbkill'])) echo '<tr style="background-color:#6b8e23">'; else echo '<tr>';
					echo '<td>cach_kill</td><td>Имя: <input name="klGlNm" value="'.$_SESSION['klGlNm'].'"><br>Индекс1: <input name="klInx1" value="'.$_SESSION['klInx1'].'"><br>Индекс2: <input name="klInx2" value="'.$_SESSION['klInx2'].'"><br><input name="sbkill" type="submit" value="Выполнить"></td><td>';
					var_dump($_SESSION['res_cach_kill']);
					echo '</td></tr>';
					if (isset($_POST['sbzkill'])) echo '<tr style="background-color:#6b8e23">'; else echo '<tr>';
					echo '<td>cach_zkill</td><td>Имя: <input name="zklGlNm" value="'.$_SESSION['zklGlNm'].'"><br>Индекс1: <input name="zklInx1" value="'.$_SESSION['zklInx1'].'"><br>Индекс2: <input name="zklInx2" value="'.$_SESSION['zklInx2'].'"><br><input name="sbzkill" type="submit" value="Выполнить"></td><td>';
					var_dump($_SESSION['res_cach_zkill']);
					echo '</td></tr>';
					if (isset($_POST['sbord'])) echo '<tr style="background-color:#6b8e23">'; else echo '<tr>';
					echo '<td>cach_order</td><td>Имя: <input name="ordGlNm" value="'.$_SESSION['ordGlNm'].'"><br>Индекс1: <input name="ordInx1" value="'.$_SESSION['ordInx1'].'"><br>Индекс2: <input name="ordInx2" value="'.$_SESSION['ordInx2'].'"><br><input name="sbord" type="submit" value="Выполнить"></td><td>';
					var_dump($_SESSION['res_cach_ord']);
					echo '</td></tr>';
					if (isset($_POST['sbordv'])) echo '<tr style="background-color:#6b8e23">'; else echo '<tr>';
					echo '<td>cach_order_rev</td><td>Имя: <input name="ordvGlNm" value="'.$_SESSION['ordvGlNm'].'"><br>Индекс1: <input name="ordvInx1" value="'.$_SESSION['ordvInx1'].'"><br>Индекс2: <input name="ordvInx2" value="'.$_SESSION['ordvInx2'].'"><br><input name="sbordv" type="submit" value="Выполнить"></td><td>';
					var_dump($_SESSION['res_cach_ordv']);
					echo '</td></tr>';
					if (isset($_POST['sbqur'])) echo '<tr style="background-color:#6b8e23">'; else echo '<tr>';
					echo '<td>cach_query</td><td>Имя: <input name="qurGlNm" value="'.$_SESSION['qurGlNm'].'"><br>Индекс1: <input name="qurInx1" value="'.$_SESSION['qurInx1'].'"><br><input name="sbqur" type="submit" value="Выполнить"></td><td>';
					...
					echo '<td>Время выполнения <br>cach_set N-раз</td><td>Количество повторов: <input name="stCount" value="'.$_SESSION['stCount'].'"><br>Имя: <input name="stCGlNm" value="'.$_SESSION['stCGlNm'].'"><br>Индекс1: <input name="stCInx1" value="'.$_SESSION['stCInx1'].'"><br>Индекс2: <input name="stCInx2" value="'.$_SESSION['stCInx2'].'"><br>Значение: <input name="stCVal" value="'.$_SESSION['stCVal'].'"><br><input name="sbCset" type="submit" value="Выполнить"></td><td>'.$_SESSION['res_cach_cset'].'</td></tr></table></form>';
				}
			}
		}
	}
